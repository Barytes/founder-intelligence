...
**Architecture:** Extend `web_workbench.app` into the unified FastAPI service. Add Python dashboard repository and pipeline runner modules that preserve the current Ruby Web app API contract. Serve the dashboard at `/`, the agent workbench at `/agent`, and prefix Agentic Core APIs under `/api/agent/*`.
...
- Modify `src/agentic-core/web_workbench/static/index.html`: add navigation back to dashboard and use `/agent/static/*`.
...
- [ ] Write failing FastAPI tests for dashboard page/static/API routes.
- [ ] Implement `DashboardRepository` until those tests pass.
- [ ] Write failing tests for refresh runner success/failure/lock behavior.
- [ ] Implement `PipelineRunner` until those tests pass.
- [ ] Add unified routes and same-origin checks to `web_workbench.app`.
- [ ] Move Agent Workbench UI calls to `/api/agent/*`.
- [ ] Add cross-navigation and style alignment.
- [ ] Update docs to make FastAPI the current HTTP entrypoint and Ruby scripts the retained pipeline executor.
- [ ] Remove the old Ruby HTTP backend files and Ruby Web app tests after the Python behavior is covered.
- [ ] Run `uv run --extra dev pytest`.
- [ ] Start the FastAPI server and smoke `GET /`, `GET /agent`, `GET /api/health`, and `GET /api/agent/default-config`.
